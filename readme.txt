...
* The bar is drawn by each screen itself, so a plugin that hides the dashboard notices cannot take it away, and on a phone it slides to show the section you are in.
* Fixed: an old bookmark to Settings > Approvals could end in a page cut halfway on servers that do not buffer the whole page. It now redirects before anything is printed.
